...
/*
import delimited $data/furniture_imports_for_decomp.csv, bindquote(strict) clear
...
save $data/furniture_import_decomp.dta, replace
*/


**Step 1: Create two datasets: quarter = t, quarter = t + 1
  ** Rename variables TEU and quantity 
...
foreach k in 239  235  231 227 {
	...
	forvalues j = 1(1)6  {
		...
		use $data/furniture_import_decomp.dta, clear
		
		keep if qdate==`i' | qdate==`k'

		order qdate

		replace shppanjivaid="." if shppanjivaid == "NA"
		destring(shppanjivaid), replace
		replace conpanjivaid="." if conpanjivaid == "NA"
		destring(conpanjivaid), replace
		...
		** Quarter = t-1
		...
		save $data/furniture_d1_totcheck_q.dta, replace
		...
		** Save "t+1" observations that do not match and "t" observations that do not match
		...
		use $data/furniture_d1_q.dta, clear

		merge 1:1 conpanjivaid shipper country using $data/furniture_d2_q.dta
		...
		** Redacted Consignee -- Intensive
		...
		save $data/furniture_d1_d2_supplier_country_m_red_q.dta, replace
		restore
		...
		save $data/furniture_d1_d2_supplier_country_m_red_q_q.dta, replace
		restore
		...
		save $data/furniture_d1_d2_supplier_country_m_red_w_q.dta, replace
		
		
		use $data/furniture_d1_d2_supplier_country_m_red_q.dta, clear
		merge 1:1 quarter type using $data/furniture_d1_d2_supplier_country_m_red_q_q.dta, nogen
		merge 1:1 quarter type using $data/furniture_d1_d2_supplier_country_m_red_w_q.dta, nogen

		save $data/furniture_d1_d2_supplier_country_m_red_q.dta, replace
		erase $data/furniture_d1_d2_supplier_country_m_red_q_q.dta
		erase $data/furniture_d1_d2_supplier_country_m_red_w_q.dta
		restore
		...
		** Intensive margin (increase and decrease)
		...
		collapse (sum) TEU_t TEU_tm1, by(quarter type)
		save $data/furniture_d1_d2_supplier_country_m_type_q.dta, replace
		restore
		...
		collapse (sum) quantity_t quantity_tm1, by(quarter type)
		save $data/furniture_d1_d2_supplier_country_m_type_q_q.dta, replace
		restore
		...
		collapse (sum) weight_t weight_tm1, by(quarter type)
		save $data/furniture_d1_d2_supplier_country_m_type_w_q.dta, replace
		
		use $data/furniture_d1_d2_supplier_country_m_type_q.dta, clear
		merge 1:1 quarter type using $data/furniture_d1_d2_supplier_country_m_type_q_q.dta, nogen
		merge 1:1 quarter type using $data/furniture_d1_d2_supplier_country_m_type_w_q.dta, nogen
		save $data/furniture_d1_d2_supplier_country_m_type_q.dta, replace
		
		erase $data/furniture_d1_d2_supplier_country_m_type_q_q.dta
		erase $data/furniture_d1_d2_supplier_country_m_type_w_q.dta
		restore
		...
		**---------
		preserve
		keep if _merge == 1
		save $data/furniture_d1_sup_nm_temp_q.dta, replace
		drop _merge shipper 
		collapse(sum) TEU_tm1 quantity_tm1 weight_tm1, by (conpanjivaid country)
		save $data/furniture_d1_sup_nm_q.dta, replace
		
		use $data/furniture_d1_sup_nm_temp_q.dta, clear
		drop _merge 
		collapse(sum) TEU_tm1 quantity_tm1 weight_tm1, by (conpanjivaid shipper country)
		save $data/furniture_d1_country_nm_step3_q.dta, replace
		restore

		preserve 
		keep if _merge == 2
		save $data/furniture_d2_sup_nm_temp_q.dta, replace
		drop _merge shipper 
		collapse(sum) TEU_t quantity_t weight_t, by (conpanjivaid country)
		save $data/furniture_d2_sup_nm_q.dta, replace 
		use $data/furniture_d2_sup_nm_temp_q.dta, clear
		drop _merge 
		collapse(sum) TEU_t quantity_t weight_t, by (conpanjivaid shipper country)
		save $data/furniture_d2_country_nm_step3_q.dta, replace
		restore 
		**---------
		...
		** Now that we're merging without supplier, we need to collapse on consignee:
		use $data/furniture_d1_sup_nm_q.dta, clear
		merge 1:1 conpanjivaid country using $data/furniture_d2_sup_nm_q.dta
		...
		**Same Country, but different supplier
		...
		collapse (sum) TEU_t TEU_tm1, by(quarter type)
		save $data/furniture_d1_d2_con_m_q.dta, replace
		restore
		...
		collapse (sum) quantity_t quantity_tm1, by(quarter type)
		save $data/furniture_d1_d2_con_m_q_q.dta, replace
		restore
		...
		collapse (sum) weight_t weight_tm1, by(quarter type)
		save $data/furniture_d1_d2_con_m_w_q.dta, replace
		
		use $data/furniture_d1_d2_con_m_q.dta, replace
		merge 1:1 quarter type using $data/furniture_d1_d2_con_m_q_q.dta, nogen
		merge 1:1 quarter type using $data/furniture_d1_d2_con_m_w_q.dta, nogen

		save $data/furniture_d1_d2_con_m_q.dta, replace
		erase $data/furniture_d1_d2_con_m_q_q.dta
		erase $data/furniture_d1_d2_con_m_w_q.dta
		restore
		...
		**---------
		...
		merge 1:m conpanjivaid country using $data/furniture_d1_country_nm_step3_q.dta
		**_m==1: 
		...
		**_m==3: are observations that did not match in step 4 above
		...
		save $data/furniture_d1_con_nm_q.dta, replace
		restore
		...
		drop _merge
		drop quantity* weight* TEU*
		merge 1:m conpanjivaid country using $data/furniture_d2_country_nm_step3_q.dta
		**_m==1: 
		**_m==2: are observations that matched in step 4 above
		**_m==3: are observations that did not match in step 4 above
		keep if _m==3
		collapse(sum) TEU_t quantity_t weight_t, by (conpanjivaid shipper)
		save $data/furniture_d2_con_nm_q.dta, replace 
		restore 
		**---------
		...
		**----------------------------------------------------------------------
		...
		use $data/furniture_d1_con_nm_q.dta, clear
		merge 1:1 conpanjivaid shipper using $data/furniture_d2_con_nm_q.dta
		...
		**Same Supplier, but different Country
		...
		collapse (sum) TEU_t TEU_tm1, by(quarter type)
		save $data/furniture_d1_d2_supp_m_q.dta, replace
		restore
		...
		collapse (sum) quantity_t quantity_tm1, by(quarter type)
		save $data/furniture_d1_d2_supp_m_q_q.dta, replace
		restore
		...
		collapse (sum) weight_t weight_tm1, by(quarter type)
		save $data/furniture_d1_d2_supp_m_w_q.dta, replace
		
		
		use $data/furniture_d1_d2_supp_m_q.dta, clear
		merge 1:1 quarter type using $data/furniture_d1_d2_supp_m_q_q.dta, nogen
		merge 1:1 quarter type using $data/furniture_d1_d2_supp_m_w_q.dta, nogen
		save $data/furniture_d1_d2_supp_m_q.dta, replace
		
		erase $data/furniture_d1_d2_supp_m_q_q.dta
		erase $data/furniture_d1_d2_supp_m_w_q.dta
		restore
		...
		**---------
		...
		save $data/furniture_d1_just_consignee_nm_q.dta, replace 
		restore
		...
		save $data/furniture_d2_just_consignee_nm_q.dta, replace 
		restore
		**---------
		...
		**----------------------------------------------------------------------
		...
		use $data/furniture_d1_just_consignee_nm_q.dta, clear 
		merge 1:1 conpanjivaid using $data/furniture_d2_just_consignee_nm_q.dta 
		...
		**Different Supplier, AND different Country
		...
		collapse (sum) TEU_t TEU_tm1, by(quarter type)
		save $data/furniture_d1_d2_diff_m_TEU_q.dta, replace
		restore
		...
		collapse (sum) weight_t weight_tm1, by(quarter type)
		save $data/furniture_d1_d2_diff_m_weight_q.dta, replace
		restore
		...
		collapse (sum) quantity_t quantity_tm1, by(quarter type)
		merge 1:1 quarter type using $data/furniture_d1_d2_diff_m_weight_q.dta, nogen
		merge 1:1 quarter type using $data/furniture_d1_d2_diff_m_TEU_q.dta, nogen
			
		save $data/furniture_d1_d2_diff_m_q.dta, replace
		erase $data/furniture_d1_d2_diff_m_weight_q.dta
		erase $data/furniture_d1_d2_diff_m_TEU_q.dta
		restore
		...
		**---------
		...
		save $data/furniture_d1_consignee_death_q.dta, replace 
		restore
		...
		**Consignee Birth
		...
		save $data/furniture_d1_consignee_birth_q.dta, replace 
		restore
		**---------
		...
		**----------------------------------------------------------------------
		...
		use $data/furniture_d1_d2_supplier_country_m_red_q.dta, clear
		append using $data/furniture_d1_d2_supplier_country_m_type_q.dta
		append using $data/furniture_d1_d2_con_m_q.dta
		append using $data/furniture_d1_d2_supp_m_q.dta
		append using $data/furniture_d1_d2_diff_m_q.dta
		append using $data/furniture_d1_consignee_death_q.dta
		append using $data/furniture_d1_consignee_birth_q.dta
		save $data/furniture_d1_d2_con_results_q.dta, replace

		
		**Merge in total checks
		merge m:1 quarter using $data/furniture_d1_totcheck_q.dta
		drop _m
		merge m:1 quarter using $data/furniture_d2_totcheck_q.dta
		drop _m
		...
		** I think because we're subsetting), so need to assert without quantity from redact.
		...
		replace quarter = `k'
		save $data/furniture_decomposition_q_dyn_`k'_`j'.dta, replace
	}
	
}

foreach k in 239 235 231 227 {
	use $data/furniture_decomposition_q_dyn_`k'_1.dta
	
	forvalues j = 2(1)6  {
		merge 1:1 quarter type using $data/furniture_decomposition_q_dyn_`k'_`j'.dta
		drop _m
		erase $data/furniture_decomposition_q_dyn_`k'_`j'.dta
	}

	save $data/furniture_decomposition_q_dyn_`k'.dta, replace
	erase $data/furniture_decomposition_q_dyn_`k'_1.dta
	
}

use $data/furniture_decomposition_q_dyn_239.dta, clear
append using $data/furniture_decomposition_q_dyn_235.dta
append using $data/furniture_decomposition_q_dyn_231.dta
append using $data/furniture_decomposition_q_dyn_227.dta
save $data/furniture_decomposition_q_dyn.dta, replace
